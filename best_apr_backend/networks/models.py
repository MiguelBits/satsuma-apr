...
class Network(AbstractBaseModel):
    ...
    def get_current_pools_info(self):
        ...
        pools_day_data = send_post_request(self.subgraph_url, json={'query': """query {
        poolDayDatas(where:{date: %s}, first: 1000){
            feesToken0
            feesToken1
            id
            pool{
            ...
            }""" % int((datetime.now().timestamp() // 86400 - 1) * 86400)}, headers=headers)

        pools_json_previous = {}

        for pool in pools_day_data['data']['poolDayDatas']:
            pools_json_previous[pool['pool']['id']] = {'feesToken0': pool['feesToken0'], 'feesToken1': pool['feesToken1']}
        ...
        for i in range(len(pools_json)):
            try:
                pools_json[i]['feesToken0'] = float(pools_json_previous[pools_json[i]['id']]['feesToken0'])
                pools_json[i]['feesToken1'] = float(pools_json_previous[pools_json[i]['id']]['feesToken1'])
            except KeyError:
                pools_json[i]['feesToken0'] = 0
                pools_json[i]['feesToken1'] = 0
